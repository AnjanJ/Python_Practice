...
# There are a few things that you need to check the first time you look at
# the inputs that you should not check on subsequent recursive calls:
...
# The first way would be to use keyword arguments. The second way would be to use a
# global variable, which you'll see in the next lecture video; however, using global variables
# is always a bit risky and thus not the best way to do this.
#
# The third way to perform checks on the inputs the first time you see them,
# but not any subsequent time, is to use a wrapper function. This wrapper function
# performs some checks, then makes a call to the recursive function.
#
# The idea of a wrapper function is really important. You'll see more wrapper functions later.
#  To introduce you to the idea, we are providing you with the wrapper function; your job is
# to write the recursive function semordnilap that the wrapper function calls.
